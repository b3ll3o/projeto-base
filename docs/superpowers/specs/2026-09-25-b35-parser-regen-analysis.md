...
- **Refs:** B29 (parser MD→workflow.json v2 IR), B30 (WORKFLOWS.md enrichment + 9/9 MDs auto-generable), B32+B33 (`archify flow enrich <json>` subcommand + 19 actor patterns), B34 (catalog 78/78 enriquecido em escala — valida parser-enriched IR end-to-end). Memory details: `.claude/projects/.../memory/b29-...`, `b30-...`, `b32-...`, `b34-...` (external à projeto).

## TL;DR

Re-avaliação pós-B33 enrichment confirma o veredito de B31: o parser-enriched (com `meta.tier/mechanism/subject` + `actorTags` + view overview) **bate topology 100% APENAS para pipelines puramente sequenciais declarados no MD**, mas tem **gaps significativos em decoration** (subtitle, output, semantic views, phases, groups, lanes, node type, sublabel semântica). Para topologias paralelas multi-lane como `flow-specialist-routing`, o parser reproduz só o pipeline declarado (2 nodes router→controller) enquanto o ground-truth adiciona 3 nodes curados da narrativa (specialist_1, specialist_2, agent_architect). Para B36+, recomendar manter ground-truth como source-of-truth para agent-workflows e usar parser para fixtures/derivação.
...
| Workflow | MD-declared nodes | Ground-truth nodes | Lanes | Ground-truth complexity |
|---|---|---|---|---|
| `flow-backend-feature` | 4 (sequencial) | 4 | 1 (agent) | 2 views, 2 phases, 2 groups |
| `flow-monorepo-change` | 2 (sequencial) | 2 | 1 (agent) | 2 views, 2 phases, 2 groups |
| `flow-specialist-routing` | 2 (sequencial; MD declara apenas `router → controller`) | 4 (ground-truth adiciona 3 hand-curados: specialist_1, specialist_2, agent_architect) | 1 (parser) vs 4 (ground-truth: controller/router/specialists/policy) | 2 views, 3 phases, 4 groups |
...
**Topology match 100% para pipelines puramente sequenciais** (backend-feature, monorepo-change) — o pipeline é extraído corretamente do MD code-fence (`NESTJS-SPECIALIST → TEST-WRITER → ...`) ou do ASCII box-art. Para `flow-specialist-routing`, parser-vs-MD é 100% (2 nodes declarados no `router → controller`), mas parser-vs-ground-truth é ~50% nodes missing (ground-truth adiciona `specialist_1`, `specialist_2`, `agent_architect` hand-curados da narrativa). **Gap é topológico, não só decoration**, para topologias paralelas multi-lane.
...
1. **Topology match é forte para os 2 pipelines sequenciais** (backend-feature, monorepo-change) — parser é source-of-truth válido para `nodes/edges/mainPath` quando o MD declara o pipeline completo. Para `flow-specialist-routing` o gap é topológico (não só decoration): ground-truth adiciona 2 specialists paralelos + 1 exception path ausentes do MD declarado
...
- Avaliar (B37.5) extensão de MD syntax para declarar nós paralelos (e.g., `→` para sequential vs `⇉` para parallel fan-out) que fecharia o gap topológico de specialist-routing
